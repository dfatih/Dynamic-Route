    ...
    for u, data in GRAPH.nodes(data=True):
        ...
        while DIFF <= (ARRIVAL - TODAY).days:
            if W_DATA['rain_sum'][DIFF] > 1:
                LIST_OF_NODES_REM.append(u)
                break  # Break the loop if the condition is met to move to the next node
            DIFF += 1
    GRAPH.remove_nodes_from(LIST_OF_NODES_REM)
    return GRAPH
...
def w_main_exec(G):
    ...
    DATE_S, DATE_E = get_date("Enter starting Date"), get_date("Enter Date of last possible arrival")
    ...
    return "Could not do a forcast ... given dates not suitable \n\n"
    ###########################-------------------------------------CHECK WEATHER CONDITONS ;

if __name__ == "__main__": 
    w_main_exec() # for testing purposes